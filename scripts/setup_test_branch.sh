...
# 預約今天 12:58:00 自動硬體通電開機，13:03:00 發送 Private 與 Wi-Fi連網等待及自動登入檢測

echo "=== 啟動測試分支 12:58 自動開機與 13:03 發送測試配置 ==="
...
# 2. 預約今天 12:58:00 硬體通電開機 (Power On)
echo "正在設定 08/09/2026 12:58:00 通電開機..."
sudo pmset schedule cancelall 2>/dev/null
sudo pmset schedule poweron "08/09/2026 12:58:00"

echo "=== 目前系統硬體預約狀態 (pmset -g sched) ==="
sudo pmset -g sched

# 3. 寫入 13:01~13:04 視訊錄影與 13:03:00 Private 發送任務至 crontab
...
(crontab -l 2>/dev/null | grep -v "send_daily_morning_card.py" | grep -v "screencapture"; \
 echo "1 13 * * * screencapture -v -V 180 -x -C \"$REC_FILE\" >/dev/null 2>&1 &"; \
 echo "3 13 * * * cd \"$PROJECT_DIR\" && PYTHONPATH=. ./venv/bin/python scripts/send_daily_morning_card.py --target \"Private\" --countdown 1 >> \"$CRON_LOG\" 2>&1"; \
...
echo ""
echo "🎉 測試分支 12:58 硬體開機與 13:03 發送測試配置成功！"
