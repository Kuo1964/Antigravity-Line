...
# 預約今天 (08/10) 09:45:00 觸發 Private 發送、Cmd+V 零偏碼自動登入檢測

echo "=== 啟動測試分支今天 09:45 發送與自動登入測試配置 ==="
...
# 2. 寫入 09:43~09:46 視訊錄影與 09:45:00 Private 發送任務至 crontab
...
(crontab -l 2>/dev/null | grep -v "send_daily_morning_card.py" | grep -v "screencapture"; \
 echo "43 9 * * * screencapture -v -V 180 -x -C \"$REC_FILE\" >/dev/null 2>&1 &"; \
 echo "45 9 * * * cd \"$PROJECT_DIR\" && PYTHONPATH=. ./venv/bin/python scripts/send_daily_morning_card.py --target \"Private\" --countdown 1 >> \"$CRON_LOG\" 2>&1"; \
...
echo ""
echo "🎉 測試分支今天 09:45 發送與 Cmd+V 自動登入測試配置成功！"
